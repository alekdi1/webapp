...
@Component
export default class CooperationComponent extends Base {
    ...
    private async submit () {
        ...
        if (!allValidated) {
            let elRef: any = null
            let type = "input"
            if (errors.emptyCompanyName) {
                elRef = this.$refs["input-company-name"]
            } else if (errors.invalidTaxId) {
                elRef = this.$refs["input-tax-id"]
        ...
        this.loading = true
        const { firstname, lastname, email, phone, companyName, taxId, shopName, selectedMethods, uploadFiles } = this.form
...
class RegisterForm {
    ...
    get errors () {
        return {
            ...
            emptyCompanyName: ((n) => {
                if (n.length === 0) return "กรุณาระบุชื่อบริษัท"
                return null
            })(this.companyName),
            ...
            invalidTaxId: ((t) => {
                if (!ValidateUtils.validTaxId(t)) return "กรุณาระบุหมายเลขประจำตัวผู้เสียภาษีให้ถูกต้อง"
                return null
            })(this.taxId),
