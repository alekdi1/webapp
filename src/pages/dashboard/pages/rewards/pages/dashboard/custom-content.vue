        ...
        <div class="transaction-list">
            ...
            <v-tabs-items v-model="summaryTab">
                <v-tab-item :value="0">
                    <template>
                        <summary-card-custom
                            ref="custom-summary-card-redeem"
                            :dateRange="dateRange"
                            :disabled="loading"
                            :loading="loading"
                        ...
                        <div v-else>
                            ...
                            <v-card @click="selectSummaryCard(idx, 'custom-summary-card-redeem', item.date, summaryRedeem)"
                ...
                <v-tab-item :value="1">
                    <template>
                        <summary-card-custom
                            ref="custom-summary-card-transaction"
                            :dateRange="dateRange"
                            :disabled="loading"
                            :loading="loading"
                        ...
                        <div v-else>
                            <v-divider class="mt-3" />
                            <v-card @click="selectSummaryCard(idx, 'custom-summary-card-transaction', item.date, summaryTransaction)"
                ...
                <v-tab-item :value="2">
                    <template>
                        ...
                        <div v-else>
                            <v-divider class="mt-3" />
                            <v-card @click="selectSummaryCard(idx, 'custom-summary-card-sales', item.date, summarySales)"
                ...
                <v-tab-item :value="3">
                    <template>
                        ...
                        <div v-else>
                            <v-divider class="mt-3" />
                            <v-card @click="selectSummaryCard(idx, 'custom-summary-card-void', item.date, summaryVoid)"
...
import CustomSummaryCard from "./view/custom-summary-type.vue"
...
@Component({
    components: {
        "summary-card-custom": CustomSummaryCard,
    }
})
export default class CustomCcontentView extends Base {
    ...
    @Watch("trigger", { "immediate": true })
    private async onDateRangeChange () {
        if (this.trigger) {
            this.startDate = this.dateRange[0]
            this.endDate = moment(this.dateRange[1], DF).locale(this.lang).format(DF)
            this.dataLoaded = []
    ...
    private async onTabChange () {
        ...
        switch (this.summaryTab) {
            ...
            case 0: {
                ...
                refKey = "custom-summary-card-redeem"
                break
            }
            ...
            case 1: {
                await this.getTransactions()
                refKey = "custom-summary-card-transaction"
                break
            }
            ...
            case 2: {
                await this.getTransactions()
                refKey = "custom-summary-card-sales"
                break
            }
            ...
            case 3: {
                ...
                refKey = "custom-summary-card-void"
                break
            }
        }
    ...
    private async getVoidHistory () {
        ...
        try {
            ...
            const labels: Date[] = []
            const now = moment(this.endDate, DF).format(DF)
            const processDate = moment(this.startDate, DF)
    ...
    private async getTransactions () {
        ...
        try {
            ...
            const rs = await RewardServices.getTransactions({
                endDate: moment(this.endDate, DF).format(DF),
                startDate: moment(this.startDate, DF).format(DF),
                partnerCode: config.partnerCode,
                the1BizBranchCode: config.branchCode1Biz,
                paginate: {
                    itemPerPage: PAGE_SIZE,
    ...
    private selectSummaryCard (idx: number, refKey: string, currentDate: Date, group: SummaryGroup<SummaryVoidItem | SummarySalesItem | SummaryRedeemItem | SummaryTransactionItem>) {
        ...
        const drange = Math.abs(moment(this.endDate, DF).diff(moment(this.startDate, DF), "days"))
        this.selectCardDataIndex(refKey, drange - idx)
    }
