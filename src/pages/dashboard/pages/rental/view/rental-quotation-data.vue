...
export default class RentalData extends Vue {
    @Prop({ default: () => new RentalModel.Quotation() })
    private data!: RentalModel.Quotation
    ...
    private get startEndDate() {
        const start = TimeUtils.convertToLocalDateFormat(moment(this.data.startDate))
        const end = TimeUtils.convertToLocalDateFormat(moment(this.data.endDate))
        return `${start} - ${end}`
    }
    ...
    private get datas2(): ViewData[] {
        ...
        if (this.data.pricePerMonth) {
            datas.push({
                label: lang("ราคาค่าเช่า (GP) Minimum", "Rental price (GP) Minimum"),
        ...
        datas.push({
            label: lang("ระยะเวลา", "Time range"),
            value: this.startEndDate
        })
    ...
    private get isRenew () {
        const status = RentalServices.RENTAL_STATUS
        return [status.renew, status.expiring].includes(this.status)
    }
